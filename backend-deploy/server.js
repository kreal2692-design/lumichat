...
// ============================================
// IN-APP PURCHASE TRANSACTIONS API
// ============================================

// Transaction kaydet ve jetonları ekle
app.post('/api/transactions', async (req, res) => {
  try {
    const { userId, productId, purchaseToken, platform = 'android' } = req.body;
    
    if (!userId || !productId || !purchaseToken) {
      return res.status(400).json({ error: 'Eksik parametre: userId, productId, purchaseToken gerekli' });
    }

    // Token paketleri tanımı
    ...
    // Aynı purchase token daha önce kullanılmış mı kontrol et (double spending prevention)
    const { data: existing } = await supabase
      .from('transactions')
      .select('id')
    ...
    if (existing) {
      return res.status(409).json({ error: 'Bu satın alma zaten işlendi' });
    }
    ...
    if (txnError) {
      ...
      return res.status(500).json({ error: 'Transaction kaydedilemedi' });
    }

    // Token paketi ise jetonları ekle
    if (product.type === 'token') {
      ...
      if (updateError) {
        ...
        return res.status(500).json({ error: 'Jetonlar eklenemedi' });
      }

      console.log(`[IAP] ${userId} kullanıcısına ${product.amount} jeton eklendi. Yeni bakiye: ${newTokens}`);

      return res.json({
        success: true,
        transaction: txn,
        tokens: newTokens,
        message: `${product.amount} jeton hesabınıza eklendi!`
      });
    }

    // Premium abonelik ise premium süresini ekle
    if (product.type === 'subscription') {
      ...
      if (updateError) {
        ...
        return res.status(500).json({ error: 'Premium eklenemedi' });
      }

      console.log(`[IAP] ${userId} kullanıcısına ${product.months} ay premium eklendi. Bitiş: ${startDate.toISOString()}`);

      return res.json({
        success: true,
        transaction: txn,
        premium_until: startDate.toISOString(),
        message: `${product.months} ay premium hesabınıza eklendi!`
      });
    }
...
app.get('/api/transactions/:userId', async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({ error: 'Transaction geçmişi alınamadı' });
    }
